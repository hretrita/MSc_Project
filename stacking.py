...
# ROC curve
pred_rf = rf.predict(v_probs)
pred_svm = svm.predict(v_probs)
pred_lr = lr.predict(v_probs)
metrics.plot_roc_curve(rf, v_probs, pred_rf, ax=ax)
metrics.plot_roc_curve(svm, v_probs, pred_svm, ax=ax)
metrics.plot_roc_curve(lr, v_probs, pred_lr, ax=ax)
plt.show()
